...
public class SnpSiftCmdDbNsfp extends SnpSift {
    ...
    ArrayList<VcfEntry> annotate(boolean createList) {
        ...
        for (VcfEntry vcfEntry : vcfFile) {
            try {
                // Show header?
                if (showHeader) {
                    processVcfHeader(vcfFile); // Add VCF header
                    showHeader = false;
                    checkFieldsToAdd(); // Check that the fields we want to add are actually in the database
                }
     ...
     *
     * @param info Add values to 'info' map. Note that we may have some previous values in the map
     * @return True if entries were added to the map
     */
    public boolean annotate(Variant variant, Map<String, String> info) {
        ...
        // Find entry in dbNSFP
        Collection<DbNsfpEntry> dbEntries = dbNsfp.query(variant);
        if (dbEntries == null || dbEntries.isEmpty()) {
            if (!annotateMissing) return false; // No need to annotate missing entries? Then we are done
            if (dbEntries == null) dbEntries = new LinkedList<>(); // We need to annotate missing entries
        }
        ...
        for (String fieldKey : fieldsToAdd.keySet()) {
            ...
            // Add annotations to 'info' hash
            if (infoValue != null) {
                String oldInfo = info.get(fieldKey);

                String newInfo;
                if (oldInfo == null || oldInfo.isEmpty()) newInfo = infoValue;
                else newInfo = oldInfo + "," + infoValue;

                info.put(fieldKey, newInfo);
                annotated = true;
            }
        }
    ...
    public void checkFieldsToAdd() throws IOException {
        ...
        for (String fieldKey : fieldsToAdd.keySet())
            if (!dbNsfp.hasField(fieldKey))
                Log.fatalError("dbNsfp does not have field '" + fieldKey + "' (file '" + dbFileName + "')");
    }
    ...
    public List<VcfEntry> run(boolean createList) {
        if (genomeVersion.isEmpty() && dbFileName == null)
            usage("You must provide either a genome name or a database path (i.e. '-g' or '-db' command line options)");
